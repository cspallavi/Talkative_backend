...
$y="yes";
$q1=mysqli_query($conn,"select * from `message` where(`msgsenderid`='$sessionid' and `msgreceiverid`='$view_userid')or(`msgsenderid`='$view_userid' and `msgreceiverid`='$sessionid')order by `m_id` desc;");
...
$q6=mysqli_query($conn,"select * from `friends` where(`friend_request_sent_from`='$sessionid' and `friend_request_sent_to`='$view_userid')or(`friend_request_sent_from`='$view_userid' and  `friend_request_sent_to`='$sessionid');");
	...
	$fid=$f6['fid'];
	$response=1;
	$list[]=array('response'=>$response,'fid'=>$fid,'sessionid'=>$sessionid,'m_id'=>$mid,'senderid'=>$msgsenderid,'message'=>$message,'sender_name'=>$sender_name,'image_path'=>$image_path);
}
}
else
{
	$response=2;
	$list[]=array('response'=>$response,'fid'=>null,'sessionid'=>null,'m_id'=>null,'senderid'=>null,'message'=>null,'sender_name'=>null,'image_path'=>null);
}
