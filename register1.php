...
if((!empty($obj->email))and(!empty($obj->username))and(!empty($obj->password))and(!empty($obj->cpassword)))
{
	...
	if(mysqli_num_rows($q1)==0)
	{
		if(strcmp($password,$cpassword)==0)
		{
			...
			$q99=mysqli_query($conn,"select * from `users` where `email`='$email' and `password`='$password';");
			$f99=mysqli_fetch_assoc($q99);
			$userid=$f99['userid'];
			$txt="being talkative";
			$q100=mysqli_query($conn,"insert into `user_detail`(`userid`,`gender`,`max_qualification`,`studied_in`,`workplace`,`lives_in`,`marital_status`,`user_status`)values('$userid','','','','','','','$txt');");
